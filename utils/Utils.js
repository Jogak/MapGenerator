...
export function getPossiblePointRiver(a, origin_x, origin_y) {
    var points = [];
    var cpt = 0;
    if (hexIsAccessible(a, origin_x - 1, origin_y)) {
        points[cpt] = (new Point(origin_x - 1, origin_y));
        cpt += 1;
    }
    if (hexIsAccessible(a, origin_x + 1, origin_y)) {
        points[cpt] = (new Point(origin_x + 1, origin_y));
    ...
    if (hexIsAccessible(a, origin_x, origin_y - 1)) {
        points[cpt] = (new Point(origin_x, origin_y - 1));
        cpt += 1;
    }
    if (hexIsAccessible(a, origin_x, origin_y + 1)) {
        points[cpt] = (new Point(origin_x, origin_y + 1));
...
export function drawRiver(points, ctx) {
    var f = new Layout(Layout.flat, new Point(15.0, 15.0), new Point(15, 0));
    ctx.beginPath();
    ctx.lineWidth = '3';
